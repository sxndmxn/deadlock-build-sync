...
from deadlock_build_sync.offline.core_policy_config import SELECTION_FOLDS
from deadlock_build_sync.offline.production_sources import MINIMUM_CORE_SUPPORT
...
from deadlock_build_sync.offline.production_storage import (
    _atomic_write,
    _folds_by_match,
)

__all__ = ["_atomic_write", "_folds_by_match"]
...
def _core_decisions(con: duckdb.DuckDBPyConnection, hero_id: int) -> pl.DataFrame:
    ...
    if not isinstance(frame, pl.DataFrame):
        raise TypeError("decision query did not return a table")
    return frame
