...
from tools.comparisons.build_guides.evidence import collect
from tools.comparisons.build_guides.paths import plan
from tools.comparisons.build_guides.render import markdown, render
from tools.comparisons.build_guides.schema import load_guide
...
def verify_evaluation(runs: Path, nominations: list[dict]) -> dict:
    ...
    for name, expected in report["source_sha256"].items():
        if fingerprint(Path("tools/comparisons/identity_paths") / name) != expected:
