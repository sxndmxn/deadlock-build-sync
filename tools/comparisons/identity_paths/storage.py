...
from tools.comparisons.core_discovery.discover import source_fingerprints
...
def producer_hashes() -> dict:
    ...
    paths += list(Path("src/deadlock_build_sync/offline").glob("discovery_*.py"))
    paths += [
        Path("src/deadlock_build_sync/offline/purchase_order.py"),
        Path("tools/comparisons/qdfm/state.py"),
    ]
    return {
...
# Public inspection helpers used by independent comparison runs.
__all__ = ["source_hashes"]
