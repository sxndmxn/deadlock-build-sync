...
def test_loader_rejects_duplicate_or_mismatched_hero_sets(tmp_path: Path) -> None:
    path = tmp_path / "build-evidence.json"
    duplicate_hero = make_evidence_document()
    ...
    duplicate_request = make_evidence_document()
    ...
    missing_request = make_evidence_document()
...
def test_loader_rejects_invalid_build_path_identity(
    ...
    document = make_evidence_document()
    get_first_build(document)[field] = value
    path = tmp_path / "build-evidence.json"
    write_fingerprinted_evidence(path, document)
...
def test_loader_rejects_invalid_path_cohorts_or_items(
    ...
    document = make_evidence_document()
    get_first_build(document).update(changes)
    path = tmp_path / "build-evidence.json"
    write_fingerprinted_evidence(path, document)
...
def test_loader_rejects_item_fold_denominator_drift(tmp_path: Path) -> None:
    document = make_evidence_document()
    build = get_first_build(document)
...
def test_loader_rejects_malformed_heroes_and_duplicate_paths(tmp_path: Path) -> None:
    ...
    no_name = make_evidence_document()
    ...
    no_builds = make_evidence_document()
    ...
    duplicate_paths = make_evidence_document()
