...
from tools.mutation_gate import COUNT_KEYS, collect_mutation_issues, main
...
def test_mutation_gate_accepts_only_a_complete_kill() -> None:
    assert collect_mutation_issues(_summary()) == ()

    issues = collect_mutation_issues(_summary(killed=1, survived=1, no_tests=1))
...
def test_mutation_gate_rejects_empty_unclassified_and_bad_counts() -> None:
    assert collect_mutation_issues(_summary(total=0, killed=0)) == (
        "no mutants were generated",
    )
    assert collect_mutation_issues(_summary(killed=2)) == (
        "classified mutants are 2; required total 3",
        "detected mutants are 2; required total 3",
    )
    assert collect_mutation_issues(_summary(killed=2, timeout=1)) == ()

    for value in (True, -1, "1"):
        summary = _summary()
        summary["total"] = value
        with pytest.raises(TypeError, match="non-negative integer"):
            collect_mutation_issues(summary)
