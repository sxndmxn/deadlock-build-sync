...
def calculate_rank_cutoffs(minimum: int, maximum: int, mode: str) -> tuple[int, ...]:
    if mode not in {"auto", "off"}:
        raise ValueError("rank expansion must be auto or off")
    RankRange(parse_badge_rank(minimum), parse_badge_rank(maximum))
...
def parse_badge_rank(badge: int) -> Rank:
...
class HeroCohort:
    ...
    def rank_range(self) -> RankRange:
        return RankRange(
            parse_badge_rank(self.minimum_badge), parse_badge_rank(self.maximum_badge)
        )
    ...
    def parse(cls, value: object) -> HeroCohort:
        ...
        minimum = _require_integer(row.get("minimum_badge"), "effective minimum badge")
        maximum = _require_integer(row.get("maximum_badge"), "effective maximum badge")
        mode = str(row.get("rank_expansion"))
        start = _require_integer(
            history[0].get("minimum_badge"), "starting minimum badge"
        )
        try:
            cutoffs = calculate_rank_cutoffs(start, maximum, mode)
...
def _validate_attempt(
    ...
    for key in (
        ...
        _require_integer(attempt.get(key), key)
    builds = _require_integer(attempt.get("supported_builds"), "supported builds")
    if bool(builds) != last:
