...
from .discovery_config import LEIDEN_RANDOM_SEEDS, MINIMUM_GROUP_JACCARD
from .discovery_ownership import calculate_core_ownership_mask
from .discovery_types import CoreDiscoveryCandidate, CoreGroupingResult


def calculate_ownership_similarities(
    candidates: list[CoreDiscoveryCandidate], matrix: np.ndarray, items: tuple[int, ...]
) -> np.ndarray:
    index = {item: column for column, item in enumerate(items)}
    masks = [
        calculate_core_ownership_mask(
            matrix, tuple(index[item] for item in candidate["items"])
        )
    ...
    for first, second in combinations(range(len(candidates)), 2):
        ...
        if jaccard >= MINIMUM_GROUP_JACCARD:
            result[first, second] = result[second, first] = jaccard
    return result


def merge_complete_link_groups(
    weights: np.ndarray, eligible: np.ndarray
) -> list[list[int]]:
...
def group_core_candidates(
    candidates: list[CoreDiscoveryCandidate],
    matrix: np.ndarray | None = None,
    items: tuple[int, ...] = (),
) -> CoreGroupingResult:
    weights = (
        calculate_item_similarities(candidates)
        if matrix is None
        else calculate_ownership_similarities(candidates, matrix, items)
    )
    ...
    for seed in LEIDEN_RANDOM_SEEDS:
        if not len(first):
    ...
    if matrix is not None:
        eligible &= weights > 0
    groups = merge_complete_link_groups(weights, eligible)
    return {
        "groups": groups,
        "seeds": assignments,
        "edges": [
...
def calculate_item_similarities(candidates: list[CoreDiscoveryCandidate]) -> np.ndarray:
