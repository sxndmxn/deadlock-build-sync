...
The first-order transition model predicts the exact next item at rank one in **{sequence_all_transition["top1_accuracy"]:.1%}** of **{int(sequence_all_transition["test_transitions"]):,}** held-out transitions and places it in the top five **{sequence_all_transition["top5_accuracy"]:.1%}** of the time. Conditioning that transition on the build's first purchase raises top-one accuracy to **{sequence_all_first_item["top1_accuracy"]:.1%}** and top-five to **{sequence_all_first_item["top5_accuracy"]:.1%}**, with **{sequence_all_first_item["context_coverage"]:.1%}** context coverage. After removing direct component-to-upgrade transitions, first-purchase conditioning raises top-one accuracy from **{sequence_non_component_transition["top1_accuracy"]:.1%}** to **{sequence_non_component_first_item["top1_accuracy"]:.1%}** and top-five from **{sequence_non_component_transition["top5_accuracy"]:.1%}** to **{sequence_non_component_first_item["top5_accuracy"]:.1%}**, versus **{sequence_non_component_position["top1_accuracy"]:.1%}** top-one for the hero-and-purchase-position baseline. Early purchase context therefore carries reproducible archetype information beyond game stage and mandatory upgrades. This is imitation evidence, not proof that the observed sequence is optimal; use it as a path-coherence term after candidate eligibility and as the baseline an XGBoost ranker must beat.

### XGBoost challenger protocol

XGBoost is appropriate as a **contextual next-item ranker**, not as a direct item win-rate score. Construct one learning-to-rank query per player-match purchase opportunity, with one row per legal candidate item and the actually purchased next item labeled relevant. Start with `rank:ndcg` or `rank:pairwise`; preserve complete query groups when splitting by match chronology.

Candidate features may include:

- hero, first purchase, previous purchases, owned-item set, purchase position, phase, elapsed time, and prior catalog spend;
- prior-snapshot net worth and team lead plus explicit missingness indicators—never final net worth or a future snapshot;
- rank family and calibration state;
- candidate item identity, tier, incremental cost after component credit, slot, active status, upgrade chain, and inventory legality;
- source-backed hero ability scaling and item mechanics; and
- enemy composition only for a separately audited situational model.

The challenger must beat the first-item-conditioned transition model on chronological NDCG/MRR and top-1/3/5 accuracy, retain coverage, improve held-out path co-purchase coherence, and keep every emitted path legal and temporally stable. A gain in final-win prediction does **not** pass this gate. If XGBoost is later used for outcomes, it belongs only inside a cross-fitted propensity/outcome nuisance model with overlap diagnostics and doubly robust estimation—not as the build ranker itself.
