CREATE TABLE first_purchases AS
WITH firsts AS (
    SELECT
        p.match_id,
        p.player_slot,
        p.team_id,
        p.hero_id,
        p.assigned_lane,
        p.average_badge,
        p.won,
        p.start_time,
        p.duration_s,
        p.final_net_worth,
        p.calibration,
        p.item_id,
        p.buy_time,
        p.sold_time,
        p.imbued_ability_id,
        p.item_name,
        p.class_name,
        p.tier,
        p."cost",
        p.slot,
        p.active,
        p.unique_item,
        p.component_items_json,
        p.own_net_worth_at_buy,
        p.state_observed_at_s,
        p.event_order,
        p.same_second_purchase_count,
        p.item_purchase_ordinal,
        f.fold,
        CASE
            WHEN p.buy_time < 540 THEN 0
            WHEN p.buy_time < 1200 THEN 1
            WHEN p.buy_time < 1800 THEN 2
            ELSE 3
        END AS phase
    FROM purchases AS p
    INNER JOIN match_folds AS f ON p.match_id = f.match_id
    WHERE p.item_purchase_ordinal = 1
),

own_state AS (
    SELECT
        f.match_id,
        f.player_slot,
        f.team_id,
        f.hero_id,
        f.assigned_lane,
        f.average_badge,
        f.won,
        f.start_time,
        f.duration_s,
        f.final_net_worth,
        f.calibration,
        f.item_id,
        f.buy_time,
        f.sold_time,
        f.imbued_ability_id,
        f.item_name,
        f.class_name,
        f.tier,
        f."cost",
        f.slot,
        f.active,
        f.unique_item,
        f.component_items_json,
        f.own_net_worth_at_buy,
        f.state_observed_at_s,
        f.event_order,
        f.same_second_purchase_count,
        f.item_purchase_ordinal,
        f.fold,
        f.phase,
        s.team_net_worth AS own_team_net_worth,
        s.observed_players AS own_team_observed_players
    FROM firsts AS f
    ASOF LEFT JOIN team_snapshots AS s
        ON
            f.match_id = s.match_id
            AND f.team_id = s.team_id
            AND f.buy_time >= s.stat_time
),

both_states AS (
    SELECT
        o.match_id,
        o.player_slot,
        o.team_id,
        o.hero_id,
        o.assigned_lane,
        o.average_badge,
        o.won,
        o.start_time,
        o.duration_s,
        o.final_net_worth,
        o.calibration,
        o.item_id,
        o.buy_time,
        o.sold_time,
        o.imbued_ability_id,
        o.item_name,
        o.class_name,
        o.tier,
        o."cost",
        o.slot,
        o.active,
        o.unique_item,
        o.component_items_json,
        o.own_net_worth_at_buy,
        o.state_observed_at_s,
        o.event_order,
        o.same_second_purchase_count,
        o.item_purchase_ordinal,
        o.fold,
        o.phase,
        o.own_team_net_worth,
        o.own_team_observed_players,
        s.team_net_worth AS enemy_team_net_worth,
        s.observed_players AS enemy_team_observed_players
    FROM own_state AS o
    ASOF LEFT JOIN team_snapshots AS s
        ON
            o.match_id = s.match_id
            AND (1 - o.team_id) = s.team_id
            AND o.buy_time >= s.stat_time
)

SELECT
    match_id,
    player_slot,
    team_id,
    hero_id,
    assigned_lane,
    average_badge,
    won,
    start_time,
    duration_s,
    final_net_worth,
    calibration,
    item_id,
    buy_time,
    sold_time,
    imbued_ability_id,
    item_name,
    class_name,
    tier,
    "cost",
    slot,
    active,
    unique_item,
    component_items_json,
    own_net_worth_at_buy,
    state_observed_at_s,
    event_order,
    same_second_purchase_count,
    item_purchase_ordinal,
    fold,
    phase,
    own_team_net_worth,
    own_team_observed_players,
    enemy_team_net_worth,
    enemy_team_observed_players,
    own_team_net_worth - enemy_team_net_worth AS team_net_worth_lead,
    buy_time - state_observed_at_s AS state_age_s,
    sum("cost") OVER (
        PARTITION BY match_id, player_slot
        ORDER BY buy_time
        RANGE BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
    ) AS prior_catalog_spend,
    count(*) OVER (
        PARTITION BY match_id, player_slot
        ORDER BY buy_time
        RANGE BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
    ) AS prior_purchase_count
FROM both_states
ORDER BY hero_id, match_id, player_slot, buy_time, item_id;
