...
impl<'data> Decoder<'data> {
    ...
    fn add_value_bytes(&mut self, length: usize) -> Result<()> {
        self.value_bytes = checked_total([self.value_bytes, length])?;
        Ok(())
    }

    fn read_value_type(&mut self) -> Result<(u8, Kv3Flag)> {
        let tag = self.buffers.types.read_byte()?;
        if tag & 0x80 == 0 {
            return Ok((tag, Kv3Flag::None));
        }
        let mut encoded_flag = self.buffers.types.read_byte()?;
    ...
    fn read_typed_value(&mut self, tag: u8, flag: Kv3Flag, depth: usize) -> Result<Kv3Value> {
        ...
        let kind = match tag {
            1 => Kv3Kind::Null,
            2 => self.read_boolean()?,
            3 => Kv3Kind::Signed(self.streams()?.doubles.read_i64()?),
            4 => Kv3Kind::Unsigned(self.streams()?.doubles.read_u64()?),
            5 => Kv3Kind::Float64(self.streams()?.doubles.read_u64()?),
            6 => Kv3Kind::String(self.read_string()?),
            7 => Kv3Kind::Blob(self.read_blob()?),
            8 | 10 | 24 | 25 => self.read_array(tag, depth)?,
            9 => self.read_object(depth)?,
            11 => Kv3Kind::Signed(i64::from(self.streams()?.integers.read_i32()?)),
            12 => Kv3Kind::Unsigned(u64::from(self.streams()?.integers.read_u32()?)),
            ...
            19 => Kv3Kind::Float32(self.streams()?.integers.read_u32()?),
            20 => Kv3Kind::Signed(i64::from(i16::from_le_bytes(
                self.streams()?.shorts.read_fixed_bytes()?,
            ))),
            21 => Kv3Kind::Unsigned(u64::from(self.streams()?.shorts.read_u16()?)),
            22 => Kv3Kind::Signed(i64::from(i8::from_le_bytes(
                self.streams()?.bytes.read_fixed_bytes()?,
            ))),
            23 => Kv3Kind::Unsigned(u64::from(self.streams()?.bytes.read_byte()?)),
            _ => return Err(Error::new(format!("Unsupported KV3 type {tag}"))),
        };
        Ok(Kv3Value { flag, kind })
    }

    fn read_boolean(&mut self) -> Result<Kv3Kind> {
        match self.streams()?.bytes.read_byte()? {
    ...
    fn read_indexed_string(&mut self, identifier: i32) -> Result<String> {
        if identifier == -1 {
        ...
        let length = self
            ...
            .len();
        self.add_value_bytes(length)?;
        Ok(self.buffers.strings[index].clone())
    }

    fn read_string(&mut self) -> Result<String> {
        let identifier = self.streams()?.integers.read_i32()?;
        self.read_indexed_string(identifier)
    }

    fn read_blob(&mut self) -> Result<Vec<u8>> {
        if self.version == 1 {
            let length = self.streams()?.integers.read_length()?;
            self.add_value_bytes(length)?;
    ...
    fn read_array(&mut self, tag: u8, depth: usize) -> Result<Kv3Kind> {
        let count = if tag >= 24 {
            usize::from(self.streams()?.bytes.read_byte()?)
        } else {
            self.streams()?.integers.read_count()?
        };
        self.check_children(count)?;
        if tag == 8 {
            return (0..count)
                .map(|_| self.read_value(depth + 1))
        ...
        let (element_tag, element_flag) = self.read_value_type()?;
        let values = (0..count)
            .map(|_| self.read_typed_value(element_tag, element_flag, depth + 1))
            .collect::<Result<Vec<_>>>();
    ...
    fn read_object(&mut self, depth: usize) -> Result<Kv3Kind> {
        let count = match &mut self.buffers.objects {
            Some(objects) => objects.read_count()?,
            None => self.streams()?.integers.read_count()?,
        };
        self.check_children(count)?;
