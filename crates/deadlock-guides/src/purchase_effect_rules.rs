pub const EFFECT_RULES: &[(&str, &str, &str)] = &[
    (
        "dealing significant weapon damage replenishes",
        "Restore ability charges",
        "You can deal weapon damage to restore ability charges",
    ),
    (
        "bullets temporarily steal max hp",
        "Health steal from shots",
        "You can keep shooting a hero to steal health",
    ),
    (
        "light or heavy melee.*spirit",
        "Spirit damage on melee",
        "You can land melee attacks to apply Spirit damage and resistance reduction",
    ),
    (
        "your melee damage will restore ammo|your next heavy melee",
        "Melee damage",
        "You can land heavy melee attacks",
    ),
    (
        "(?:when in|when you are in) close range|weapon damage when in close range",
        "Close range weapon damage",
        "You can keep shooting at close range",
    ),
    (
        "next headshot.*heal\\b",
        "Headshot damage",
        "You can land headshots for damage and healing",
    ),
    (
        "imbue an ability with lifesteal.*additional percentage",
        "Ability echo damage",
        "You can hit enemies with the imbued ability for delayed area damage",
    ),
    (
        "heals? a target allied|heal an ally",
        "Ally healing",
        "An ally needs healing or help to leave a fight",
    ),
    (
        "provides move speed and fire rate.*allies",
        "Team support",
        "Nearby allies need movement and fire rate",
    ),
    (
        "barrier.*can be self.cast",
        "Ally protection",
        "You or an ally need a barrier and help to move",
    ),
    (
        "spirit resist.*aura on a friendly",
        "Ally protection",
        "You or an ally need protection during a close fight",
    ),
    (
        "immune to bullets|deflect incoming bullets|grants bullet resist|gain additional bullet resist|whenever you take significant weapon damage",
        "Bullet defense",
        "Incoming bullet damage is the main problem",
    ),
    (
        "gain additional spirit resist|whenever you take significant spirit damage|next instance of high spirit damage",
        "Spirit defense",
        "Incoming Spirit damage is the main problem",
    ),
    (
        "temporarily death immune|untargetable and invincible",
        "Survive burst damage",
        "You need time to survive a burst of damage",
    ),
    (
        "purge all non.ultimate negative|next stun.*automatically cleansed|duration of all negative|suppress negative status",
        "Control protection",
        "Enemy control or debuffs prevent your next action",
    ),
    (
        "barrier when you are stunned",
        "Control protection",
        "You need a barrier when enemy control hits you",
    ),
    (
        "parry protects you",
        "Spell parry",
        "You can parry an incoming ability or item effect",
    ),
    (
        "successful parry against an enemy",
        "Melee parry",
        "You can parry an enemy melee attack",
    ),
    (
        "teleport",
        "Engage or escape",
        "You need a teleport to reach or leave a target",
    ),
    (
        "apply a stun|applies a stun|freezing",
        "Hard control",
        "You need to stop an enemy with a stun or freeze",
    ),
    (
        "curses an enemy|build up to a silence",
        "Silence",
        "You need to stop an enemy from using abilities",
    ),
    (
        "disarms enemy|silences their movement|prevents stamina usage",
        "Disable enemy movement or weapons",
        "You need to stop enemy movement or weapon use",
    ),
    (
        "reduce their fire rate|fire rate slowed",
        "Reduce enemy fire rate",
        "You need to reduce incoming bullet fire",
    ),
    (
        "reduces a random ability cooldown",
        "Cooldowns under Spirit damage",
        "You take repeated Spirit hits while abilities are on cooldown",
    ),
    (
        "grow larger in size",
        "Close fight durability",
        "You need resistances and melee damage in a close fight",
    ),
    (
        "reduced healing|healing reduction",
        "Healing reduction",
        "Enemy healing prevents a kill",
    ),
    (
        "spirit resist.*spirit power|spirit resist of the target|spirit resist reduced",
        "Spirit resistance reduction",
        "Your team needs more Spirit damage against a target",
    ),
    (
        "bullet (?:and spirit )?resist.*(?:enemies|headshot)|reduces bullet resist on enemies|headshot.*bullet resist|bullets reduce enemy bullet resist",
        "Bullet resistance reduction",
        "Your team needs more bullet damage against a target",
    ),
    (
        "melee.*heal",
        "Melee healing",
        "You can land melee attacks to restore health",
    ),
    (
        "heavy melee|melee damage will",
        "Melee damage",
        "You can land heavy melee attacks",
    ),
    (
        "reset the cooldown",
        "Reset abilities",
        "You need to use an ability again before its cooldown ends",
    ),
    (
        "replenishes a charge",
        "Restore ability charges",
        "You can deal weapon damage to restore ability charges",
    ),
    (
        "dash.jump.*next ability",
        "Ability boost after movement",
        "You can use an ability after a dash-jump",
    ),
    (
        "weapon damage per unique hero hit",
        "Weapon damage from abilities",
        "You can hit heroes with the imbued ability before shooting",
    ),
    (
        "cooldown",
        "Ability cooldowns",
        "You need abilities available more often",
    ),
    (
        "increase.*(?:range|effect radius)",
        "Ability range",
        "Your ability needs more range or area",
    ),
    (
        "increase.*duration",
        "Ability duration",
        "Your ability effect needs to last longer",
    ),
    (
        "imbue an ability with permanent spirit",
        "Spirit power",
        "You need more Spirit power on an ability",
    ),
    (
        "applying heal.*bonus fire rate",
        "Healing support buffs",
        "You can heal a target before they fight",
    ),
    (
        "effectiveness of your healing",
        "Healing strength",
        "Your healing needs to restore more health",
    ),
    (
        "spirit lifesteal",
        "Spirit healing",
        "You need healing while dealing Spirit damage",
    ),
    (
        "bullet.*heal|headshot.*heal",
        "Healing from shots",
        "You can hit enemies with shots to restore health",
    ),
    (
        "regeneration|grant regen|stacks to heal|heal and gain",
        "Health recovery",
        "You need to restore health between attacks",
    ),
    (
        "steal max hp",
        "Health steal from shots",
        "You can keep shooting a hero to steal health",
    ),
    (
        "bonus souls|hatch the egg|passive soul generation",
        "Soul income",
        "You can meet this item's conditions for extra souls",
    ),
    (
        "air jump or air dash",
        "Stamina movement",
        "You need more movement actions",
    ),
    (
        "effect of enemy move speed penalties",
        "Slow resistance",
        "Enemy slows prevent movement",
    ),
    (
        "movement slow|move speed reduced|slows targets",
        "Slow enemies",
        "You need to slow an enemy to keep them in reach",
    ),
    (
        "stacking spirit amp|bonus spirit damage|periodically deals spirit damage",
        "Spirit damage",
        "You can apply this item's Spirit damage effect",
    ),
    (
        "bullets.*shock|bullets will ricochet",
        "Damage to nearby enemies",
        "Enemies are close enough for damage to spread",
    ),
    (
        "fire a bullet towards any attacker",
        "Return damage",
        "Enemies will trigger the return shot by damaging you",
    ),
    ("headshot", "Headshot damage", "You can land headshots"),
    (
        "close range",
        "Close range weapon damage",
        "You can keep shooting at close range",
    ),
    (
        "weapon damage|fire rate",
        "Weapon damage",
        "You can use the item's trigger to improve weapon damage",
    ),
    ("gain a barrier", "Barrier", "You need a temporary barrier"),
];

pub const CONDITION_RULES: &[(&str, &str)] = &[
    (
        "above 65% health",
        "Your health is above 65%, so the conditional bonus is active",
    ),
    (
        "apply a stun after 2s.*airborne",
        "You need a delayed stun; airborne targets receive a longer stun",
    ),
    (
        "damage from your ultimate applies a stun",
        "Your ultimate can hit an enemy to trigger a delayed stun",
    ),
    (
        "teleport to an enemy target",
        "You need to teleport to an enemy target",
    ),
    ("teleport straight ahead", "You need to teleport forward"),
    (
        "assist or kill",
        "You can gain its bonuses from kills and assists",
    ),
    (
        "hatch the egg.*as long as you are alive",
        "You can stay alive while holding the egg to earn souls",
    ),
    (
        "target an enemy npc and consume",
        "An enemy NPC is available to consume for extra souls",
    ),
    (
        "enemy uses an ability.*consume all stacks",
        "You can store stacks from nearby enemy casts, then heal yourself",
    ),
    (
        "dealing significant spirit damage.*burn",
        "You can deal enough Spirit damage to trigger the burn and reduce healing",
    ),
    (
        "suppress negative status effects.*cannot be used while",
        "You can activate it before enemy control prevents item use",
    ),
    (
        "purge all non.ultimate negative.*cannot be used while",
        "You need to remove debuffs while you can still use items",
    ),
];

pub const STAT_RULES: &[(&[&str], &str, &str)] = &[
    (
        &["BonusAbilityCharges", "CooldownBetweenChargeReduction"],
        "Ability charges",
        "You need more uses of a charged ability",
    ),
    (
        &["CooldownReduction", "ItemCooldownReduction"],
        "Ability cooldowns",
        "You need abilities available more often",
    ),
    (
        &["BonusAbilityDurationPercent"],
        "Ability duration",
        "Your ability effect needs to last longer",
    ),
    (
        &["TechRangeMultiplier"],
        "Ability range",
        "Your ability needs more range or area",
    ),
    (
        &["NonPlayerBonusWeaponPower"],
        "NPC farming",
        "You need more weapon damage against NPCs",
    ),
    (
        &["BonusBulletSpeedPercent"],
        "Bullet speed",
        "You need faster bullets",
    ),
    (
        &["BonusClipSizePercent"],
        "Magazine capacity",
        "You need more shots before a reload",
    ),
    (
        &["AbilityLifestealPercentHero"],
        "Spirit healing",
        "You need healing while dealing Spirit damage",
    ),
    (
        &["BonusHealthRegen"],
        "Health recovery",
        "You need health recovery between attacks",
    ),
    (&["BonusHealth"], "Health", "You need more health"),
    (
        &["BonusSprintSpeed"],
        "Travel speed",
        "You need faster travel between fights",
    ),
    (
        &["Stamina"],
        "Stamina movement",
        "You need more movement actions",
    ),
    (
        &["TechPower", "TechPowerPercent"],
        "Spirit power",
        "You need more Spirit power",
    ),
    (
        &["BonusFireRate"],
        "Fire rate",
        "You need to fire more bullets in the same time",
    ),
];
